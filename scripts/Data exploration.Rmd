...
plot_histogram <- function(df, var, proj_name, bins = 30, fill_color = "blue") {
  df_clean <- clean_df(df, var)

  hist <- ggplot(df_clean, aes(x = .data[[var]])) +
    geom_histogram(aes(y = ..density..), bins = bins,
                   alpha = 0.6, fill = fill_color, color = "navy") +
...
# Boxplot
plot_boxplot <- function(df, numeric_var, group_var, proj_name, fill_color = "darkgreen") {
...
plot_bar <- function(df, var, proj_name) {
  ...
  bar <- ggplot(df_clean, aes(x = .data[[var]], fill = .data[[var]])) +
    geom_bar() +
  ...
  print(bar)
  ggsave(filename = paste0("Barplot_", var, "_", proj_name ,".png"), plot = bar, path = here(plot_folder), width = width, height = height)
}
